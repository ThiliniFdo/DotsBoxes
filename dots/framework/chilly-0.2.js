...
var Chilly = module.exports = (function() {
    ...
    Chilly = Object.create(null, {
                ...
                init: function(ctx) {
                    ...
                    this.session = (function() {
                        return {
							...
							/**
                             * Destroy a session param
                             * @param key
                             */
                            destroy: function(key) {
                                if(request.req.session) {
                                    delete request.req.session[key];
                                    return;
                                }
                                throw {
                                    name: 'ChillySessionException',
					...
					if(Chilly.Clients[this.session.get('clientId')])
					{
						console.log("counter updated : "+this.session.get('clientId'));
        ...
        push: {
            value: function(obj) {
                ...
                obj['recipients'].forEach(function(clientId){
					console.log("clientId = " + clientId + " + obj.channel = " + obj.channel + " : " + this.getConnection(clientId, obj.channel));
                    ...
                    if(this.getConnection(clientId, obj.channel)) {
						//console.log("clientId = " + clientId + " + obj.channel = " + obj.channel);
        ...
        removeClient: {
            value: function(clientId) {
				console.log("removeClient called : " + clientId);
				console.log("GAMES : " + JSON.stringify(this.Games));
				
				for(var id in this.Games){
					var users = this.Games[id].getRecipients();
					console.log("USERS : " + JSON.stringify(users));
					console.log("DELETING : "+ this.Games[id].players[clientId]);
					this.Games[id].removePlayer(clientId);
					
					if(this.Games[id].getRecipients().length == 0){
            ...
            value: function() {
                setInterval(function() {
                    Object.keys(Chilly.Clients).forEach(function(id) {
						console.log(new Date().getTime() - this.Clients[id]['lastRequest'].getTime());
        ...
        addConnection: {
            value: function(clientId, channel, request) {
                request.req.pause();
				if(this.Connections != null && this.Connections[clientId] != null){
        ...
        getConnection: {
            value: function(clientId, channel) {
				//console.log("getConnection val : " + JSON.stringify(this.Connections[clientId]));
				if(this.Connections != null && this.Connections[clientId] != null && this.Connections[clientId][channel] != null)
		...
		/**
         * Remove a game from the games object if it exists
         */
        removeGame: {
            value: function(id) {
                delete this.Games[id];
